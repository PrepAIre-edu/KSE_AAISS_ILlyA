...
| **Side effects** | None. Read-only. |
| **Example** | Input: `{"course_code": "PROB", "known_concepts": ["Bayes' rule", "Combinations", "Permutations"], "max_residual_concepts": 5}`. Full output: `{"course_code": "PROB", "ects": 4, "total_concepts": 34, "covered_concepts": ["Bayes' rule", "Combinations", "Permutations"], "residual_concepts": ["addition rules", "Basics of logic", "Cartesian product", "combinatorics", "combinatorics in probability word problems", "Conditional probability", "Euler diagrams", "Event space", "events", "Expected value", "independent events", "Logical operations", "Logical propositions", "multiplication", "negations to claims", "Number of Elements in a Finite Set", "posteriori probability", "Probability", "Probability definition", "Random distributions", "Rules of probability", "rules of probability Preparation lecture materials 8", "set operations", "set theory", "Sets", "standard deviation", "subsets", "Total probability law (chain rule", "true/false propositions", "truth tables", "Variance"], "residual_count": 31, "waivable": false, "note": "31 concept(s) not recognized among the 3 known_concepts given; exceeds the max_residual_concepts=5 limit."}`. (Captured live, 2026-08-19 — the 3 known concepts leave 31 of PROB's 34 concepts uncovered, so the waiver is denied. The one visibly odd entry, "rules of probability Preparation lecture materials 8", is a known table-layout extraction artifact from the source PDF — see `docs/DESIGN_RATIONALE.md`, not a bug in this tool.) |
...
| **Side effects** | None. Read-only. |
| **Example** | Input: `{"course_a": "LINALG", "course_b": "PROB"}`. Full output: `{"course_a": "LINALG", "course_b": "PROB", "ects_a": 3, "ects_b": 4, "concept_count_a": 38, "concept_count_b": 34, "kind_distribution_a": {"definition": 26, "metric": 1, "method": 11}, "kind_distribution_b": {"method": 2, "definition": 27, "principle": 2, "metric": 3}, "related_concepts": [], "unique_to_a": ["Algebraic view on vectors", "an inverse matrix", "Applications for SLE", "Applications for solutions for SLE", "Back to School", "concept of a linear space", "definition of a matrix (recap", "Determinant of matrix", "Determinants", "elimination", "Explicit formula for an inverse for 2×2 matrix", "Gaussian elimination", "Geometric transformations", "Geometric transformations using vectors/matrices: symmetry", "Geometrical approach in the case of a 2×2 matrix", "Inverse matrix", "inverse matrix using Jordan-Gauss method", "Jordan-Gauss method", "Linear equations", "Matrices", "matrices: addition", "Matrices of SLE", "matrix multiplication", "Matrix operations: addition", "Matrix operations: multiplication", "Multiplication of matrix by vector", "operations", "rotation", "row", "row operations", "scalar multiplication", "Solving SLE using the inverse matrix", "special matrices", "System of linear equations (SLE", "Systems of linear equations (SLE", "their applications", "transposition", "vectors"], "unique_to_b": ["addition rules", "Basics of logic", "Bayes' rule", "Cartesian product", "Combinations", "combinatorics", "combinatorics in probability word problems", "Conditional probability", "Euler diagrams", "Event space", "events", "Expected value", "independent events", "Logical operations", "Logical propositions", "multiplication", "negations to claims", "Number of Elements in a Finite Set", "Permutations", "posteriori probability", "Probability", "Probability definition", "Random distributions", "Rules of probability", "rules of probability Preparation lecture materials 8", "set operations", "set theory", "Sets", "standard deviation", "subsets", "Total probability law (chain rule", "true/false propositions", "truth tables", "Variance"], "external_prerequisites_a": [], "external_prerequisites_b": []}`. The empty `related_concepts` is expected and documented, not a bug — see the known-limitation note in [DESIGN_RATIONALE.md](DESIGN_RATIONALE.md). |
...
**On the wire**, both cases are an ordinary JSON-RPC *result* for the `tools/call` request — MCP deliberately does not use a JSON-RPC *error* for a tool-level failure, only for a protocol-level one (unknown method, malformed request). What differs is the payload shape:

```jsonc
// success — validate_study_plan with an unknown course
{"content": [{"type": "text", "text": "{\"valid\": true, ...}"}],
 "structuredContent": {"valid": true, "total_ects": 0, ...},
 "isError": false}

// failure — same tool, target_courses: ["NOPE"]
{"content": [{"type": "text",
              "text": "Error executing tool validate_study_plan: unknown_course_code: 'NOPE' is not one of the known courses (CALC, LINALG, ML, PROB)"}],
 "isError": true}
```

Two things follow from this shape, both load-bearing for our design: (1) an error response has no `structuredContent` at all — a caller checking for a specific output field on a failed call gets a clean absence, not a null/zero standing in for "didn't run"; (2) the two are visually distinguishable even to a human skimming raw transcript output (as in the defence), because `isError: true` always accompanies an "Error executing tool ..." prefix that a legitimate result never contains. A schema-constraint violation (e.g. `max_total_ects: -5`, violating `gt=0`) is caught even earlier, before the tool function body runs at all, by the MCP server's own request validation against the JSON Schema derived from the `Annotated[...]` parameter types — the tool's own code never sees an invalid value.
